...
Vishal_coded_message="joijjscrkvjjemujjyejkxnjjjjyfojjynomknowijkxnjjjgkxdjdyjlomywojkjwkmrsxojvokbxsxqjoxqsxoobjcyyx"
...
alphabets="abcdefghijklmnopqrstuvwxyz"
text="jxu evviuj veh jxu iusedt cuiiqwu yi vekhjuud"
hey=""
for i in range(len(text)):
    index=alphabets.find(text[i])
    if index >= 10:
...
alphabets="abcdefghijklmnopqrstuvwxyz"
text="bqdradyuzs ygxfubxq omqemd oubtqde fa oapq kagd yqeemsqe ue qhqz yadq eqogdq!"
hey=""
for i in range(len(text)):
    index=alphabets.find(text[i])
    if index >= 12:
...
alphabets="abcdefghijklmnopqrstuvwxyz"
text="vhfinmxklatoxkxgwxkxwteehymaxlxhewvbiaxkltlhulhexmxpxeeatoxmhkxteerlmxinihnkztfxbypxptgmmhdxxihnkfxlltzxlltyx."
hey=""
for i in range(len(text)):
    index=alphabets.find(text[i])
    if index >=7:
        new_index=index-26+7
    elif index <=6:
         new_index=index+7
    new_text=alphabets[new_index]
    hey=hey+new_text
...
text="dfcaruwfstigrvjtwhrwznjvmphotiscbxswvjipreneouhlljkpirahjibegfjdkwkedhmp"
keyword="friends"
alphabet = "abcdefghijklmnopqrstuvwxyz"
key_u="friends"*13
key=""
for i,j in zip(range(len(text)), range(len(key_u))):
        final_code_index=alphabet.find(text[i])-alphabet.find(key_u[j])
        trans_code=alphabet[(final_code_index)]
        key+=trans_code
print(key)
...
text="heyvishalfuckyoubuddy"
keyword="vishal"
alphabet = "abcdefghijklmnopqrstuvwxyz"
#print(len(keyword))
key_u="vishal"*round((len(text)/len(keyword)))
key=""
for i,j in zip(range(len(text.lower())), range(len(key_u))):
        final_code_index=alphabet.find(text[i])+alphabet.find(key_u[j])
        if final_code_index >26 :
            final_code_index=final_code_index-26
        elif final_code_index <=25:
            fina_code_index=final_code_index
        trans_code=alphabet[(final_code_index)]
        key+=trans_code
...
keywords="vishal"
alphabets = "abcdefghijklmnopqrstuvwxyz"
key_us="vishal"*round((len(texts)/len(keywords)))
keys=""
for k,l in zip(range(len(texts.lower())), range(len(key_us))):
        final_code_indexs=alphabets.find(texts[k])-alphabets.find(key_us[l])
        trans_codes=alphabet[(final_code_indexs)]
        keys+=trans_codes
print(keys)
